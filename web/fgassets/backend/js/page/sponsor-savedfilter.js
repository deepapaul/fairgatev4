...
var initPageFunctions = function() {
    ...
    _dynamicFunctionSuccess = function() {
        ...
        FgXmlHttp.post(pageVars.saveAction, {filterArr: stringifyData, contactType: 'sponsor'}, false, function(){
...
var checkForBrokenFilterCriteria = function() {
    $.getJSON(filterClubDataUrl, function(masterJsonData) {
        $('.jsonDatahidden').each(function() {
            ...
            var isBroken = false;
            isBroken = FgValidateFilter.init(masterJsonData, filterJsonData, 'sponsor');
            if (!isBroken) {
    ...
    calculateContactCount: function() {
        ...
        $('#formSponsorFilter').on('click', '.filterCount', function() {
            ...
            if (status == 'calculate') {
                ...
                $(this).attr('status','sponsorfilter');
                ...
                handleCountOrSidebarClick.updateFilter('FILTER', 'filterdisplayflag_contact'+pageVars.clubId+'-'+pageVars.contactId, '', pageVars.clubId, pageVars.contactId, '', '', pageVars.clubUrlIdentifier, 'count', '', '', '', '', 'sponsor', '', $('#'+filter_id+'_jsonData').val(), filter_id, '');
            }
        });
    }
};
