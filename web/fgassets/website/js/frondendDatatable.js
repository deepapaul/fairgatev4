...
function manipulatememberColumnFields(json) {
    ...
    var columnValues = $.parseJSON(localStorage.getItem(tableSettingValueStorage));
    var columnValueCount = (_.size($.parseJSON(localStorage.getItem(tableColumnTitleStorage)))) - 1;
    var permissionDisableFields = json.aaDataHide;
    var i = 3;
    ...
    $.each(json.aaData, function (key, values) {
        $.each(values, function (value) {
            ...
            else if (value == 'Gmembership_category') {
                if (json.aaData[key][value + "_visibility"] == 'private') {
                    json.aaData[key][value] = "<i class='fa fa-eye-slash fg-dev-Popovers' data-trigger='hover' data-placement='bottom' data-content='" + jstranslations.PrivateText + "'> </i>";
                }
            }
            ...
            switch (json.aaDataType[j]['type']) {
                ...
                case "contactname":
                    ...
                    if ((json.aaData[i]['stealthFlag'] == "1") || (json.aaData[i]['newlyaddedFlag'] == "1")) {
                        json.aaData[i][title] = "<div class='fg-contact-wrap'> " + json.aaData[i][title] + editIcons + "</div>";
                    }
                    else {
                        json.aaData[i][title] = "<div class='fg-contact-wrap'> " + "<a class='fg-dev-contactname' href='" + json.aaData[i]['click_url'] + "'>" + json.aaData[i][title] + "</a>" + editIcons + "</div>";
                    }
                    break;
                case "edit":
                    json.aaData[i][title] = '<input class="dataClass" type="checkbox" id=' + json.aaData[i]['id'] + ' name="check" data-iscompany =' + json.aaData[i]['Iscompany'] + ' data-contactclub =' + json.aaData[i]['contactclubid'] + '  value="0">';
                    break;
            }
        }
    }
        ...
        for (var j = 0, jen = json.aaDataType.length; j < jen; j++) {
            ...
            switch (json.aaDataType[j]['type']) {
                ...
                case "documentName":
                    ...
                    json.aaData[i][title] = fileIcon + '<a href="#" class="fg-dev-read fg-dev-docname ' + classname + '" data-id="' + docId + '" data_url="' + url + '" >' + json.aaData[i]['documentName'] + '</a>' + editIcon;
                    break;
...
//# sourceMappingURL=frondendDatatable.js.map