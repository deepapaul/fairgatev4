-- Function to get assigned contacts count in club executive board for aclub
DROP FUNCTION `getClubExecutiveFunctionCount`//
CREATE DEFINER=`admin`@`localhost` FUNCTION `getClubExecutiveFunctionCount`(`categoryid` INT,`roleid` INT, `functionid` INT, `clubid` INT, `executiveboard` INT, `clubtype` TEXT) RETURNS int(11)
BEGIN
	DECLARE functionCount TEXT DEFAULT '';

	SELECT count(distinct(cc.fed_contact_id)) INTO functionCount 
