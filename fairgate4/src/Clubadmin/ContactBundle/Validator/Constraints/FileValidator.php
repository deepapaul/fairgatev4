...
class FileValidator extends ConstraintValidator {
    ...
    public function validate($value, Constraint $constraint) {
        ...
        //checking whether uploaded file is in forbidden filetype
        $forbiddenFiletypes = $this->container->getParameter('forbiddenFiletypes');
        $extin = $value->getClientOriginalExtension();
        $isForbiddenFiletype = 0;
        $virusDetect = 1;
        if (in_array($extin, $forbiddenFiletypes)) {
            $isForbiddenFiletype = 1;
        }
        ...
        if ($avastScan) {
            ...
            $uploadDir = ($avastScan) ? $this->container->getParameter('avast_scan_upload_folder') : FgUtility::getUploadDir()."/temp/";
            $originalName = $value->getClientOriginalName();
            $baseName = $value->getBasename();
            //copy temporary file to our project temp folder
            copy($path, $uploadDir . $baseName);
            
            $virusCheck = new FileChecking($this->container);
            $virusDetect = $virusCheck->virusScanning($path, $baseName, $originalName);
            unlink($uploadDir . $baseName);
            if ($virusDetect != 1) {               
                $this->context->buildViolation($this->container->get('translator')->trans('VIRUS_FILE_CONTACT'))
                        ->addViolation();
                return;
            }
        }

        if (($value instanceof UploadedFile && !$value->isValid()) || ($isForbiddenFiletype == 1) ) {
            ...
            //handling forbidden filetype error
            if ($isForbiddenFiletype == 1) {
                if ($this->context instanceof ExecutionContextInterface) {
                    $this->context->buildViolation($constraint->uploadExtensionErrorMessage)
                            ->setCode(UPLOAD_ERR_EXTENSION)
                            ->addViolation();
        ...
        if ($constraint->mimeTypes) {
            ...
            if ($this->context instanceof ExecutionContextInterface) {
                $this->context->buildViolation($path)
                        ->setParameter('{{ file }}', $this->formatValue($path))
