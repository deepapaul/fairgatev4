...
class FgClubClassificationRepository extends EntityRepository
{
    ...
    public function getClassificationClasses($clubId, $catId, $contactId, $exec = true)
    {
        ...
        $q->select("c.id,c.title,c.sortOrder, c.isActive, ci18n.titleLang as title_lang, ci18n.lang,cb.id as bookMarkId")
            ->addSelect("(SELECT COUNT(ca.id) FROM AdminUtilityBundle:FgClubClassAssignment ca WHERE ca.class =c.id) AS clubCount")
            ->from('AdminUtilityBundle:FgClubClass', 'c')
            ->leftJoin('AdminUtilityBundle:FgClubClassI18n', 'ci18n', 'WITH', 'ci18n.id = c.id')
            ->leftJoin("AdminUtilityBundle:FgClubBookmarks", "cb", "WITH", "cb.class =c.id  AND cb.contact='$contactId'")
    ...
    public function saveClassificationSettings($catArr, $fnAssign, $clubDefaultLang, $clubLanguages, $clubId, $contactId, $catType, $cacheDomainName, $clubTeamId, $container)
    {
        ...
        if (count($catArr) > 0) {
            ...
            $this->executeAdminQueries($updateFullQry);
            $this->executeAdminQueries($delFinalString);
            ...
            if (count($this->bookmarkArr) > 0) {
                $this->_em->getRepository('AdminUtilityBundle:FgClubBookmarks')->handleBookmark('class', $this->bookmarkArr, $clubId, $contactId);
            }
        }
...
//                        $updateArri18n[] = "title_lang = '$langTitle'";
    ...
    public function insertLogEntries($tablename, $insertedId, $valueAfterArr, $insert = true, $langkey, $clubId, $exe = false, $contactId = '', $classId = '', $container)
    {
        ...
        foreach ($valueAfterArr as $key => $value) {
            ...
            if ($exe) {
                ...
                $this->executeAdminQueries($logQuery);
            } else {
    ...
    private function updateMainTable($clubDefaultLang, $clubId)
    {
        ...
        $this->executeAdminQueries($updateMainTable);
        return true;
    }
     ...
     * @return boolean
     */
    private function executeAdminQueries($queryString)
    {
        if($queryString != ''){
            $this->container->get('fg.admin.connection')->getAdminConnection()->executeQuery($queryString);
        }
    }
