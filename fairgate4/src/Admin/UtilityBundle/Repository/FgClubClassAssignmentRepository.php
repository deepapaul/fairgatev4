...
class FgClubClassAssignmentRepository extends EntityRepository {
    ...
    public function getClubAssignmentsOfClfn($clubIds, $clfnId, $excludeClassId = '') {
        ...
        $qb = $this->createQueryBuilder('a')
                ...
                ->leftJoin('AdminUtilityBundle:FgClubClass', 'cc', 'WITH', '( a.class=cc.id)')
                ->leftJoin('AdminUtilityBundle:FgClubClassification', 'ccl', 'WITH', '( cc.classification=ccl.id)')
    ...
    public function updateClubAssignments($assignmentArr, $federationclubId, $clubService, $contactId, $clubId, $clubObj) {
        ...
        $log = array();
        $validClassificationid = $this->_em->getRepository('AdminUtilityBundle:FgClubClassification')->getClubClassificationIds($federationclubId, $exec = true);
        $validClassification = explode(',', $validClassificationid[0]['ids']);
        $contactTable = $this->_em->getRepository('AdminUtilityBundle:FgCmContact')->find($contactId);
        $contactName = $contactTable->getName();

        foreach ($assignmentArr as $overviewclubId => $classificationArray) {
            $clubname = $this->_em->getRepository('AdminUtilityBundle:FgClub')->getClubname($overviewclubId, $clubService->get('default_lang'));
            ...
            foreach ($classificationArray as $classificationId => $classificationArrDetail) {
                ...
                if (!in_array($classificationId, $validClassification)) {
                    ...
                    $clfnObj = $this->_em->getRepository('AdminUtilityBundle:FgClubClassification')->find($classificationId);
                    ...
                    foreach ($classificationArrDetail['class'] as $classId => $classValue) {
                        ...
                        if (!in_array($classId, $validClass)) {
                            ...
                            if ($errorType == '') {
                                ...
                                $classObj = $this->_em->getRepository('AdminUtilityBundle:FgClubClass')->find($classId);
                                ...
                                if ($classValue['is_deleted'] == 1 && $classValue['is_new'] == $classId) {
                                    ...
                                    foreach ($existingassignments as $key => $value) {
                                        if ((($clubService->get('type') == 'sub_federation' && $value['sublevel_assign'] == 'assign') || $clubService->get('type') == 'federation') && $classId == $value['class_id'] && $classificationId == $value['classification_id']) {
                                            ...
                                            //Log array
                                            $log[$count + 1] = ("('$clubId','$classId','$dateToday', 'assigned club', '-', '$clubName', '-', '$contactId', '$contactName')");
                                            $clublog[$count + 1] = "('$overviewclubId','$dateToday','assigned club','$clfnTitle','$classTitle','-','$contactId','$classId', '$contactName')";
                                            break;
                                        }
                                    }
