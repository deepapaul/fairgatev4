...
class FgClubFilterRepository extends EntityRepository {
    ...
    public function getSideBarSavedFilter($contactId, $clubid) {
        $queryBuilder = $this->createQueryBuilder('f')
                ...
                ->leftJoin('AdminUtilityBundle:FgClubBookmarks', 'bm', 'WITH', '(bm.contact = :userId AND bm.club =:clubid AND bm.filter=f.id)')
                ->orderBy('f.sortOrder')
                ->Where('f.club=:clubid');
    ...
    public function getSingleSavedClubSidebarFilter($id, $contactId, $clubId) {
        $queryBuilder = $this->createQueryBuilder('f')
            ...
            ->leftJoin('AdminUtilityBundle:FgClubBookmarks', 'bm', 'WITH', '(bm.filter=f.id AND bm.contact = :contactId)')
    ...
    public function updateClubBorkenFilter($id, $isBroken) {
        ...
        $filterObj = $conn->getRepository('AdminUtilityBundle:FgClubFilter')->find($id);
