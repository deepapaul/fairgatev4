...
class FgPermissions
{
    ...
    private function iterationFoumTabs($allowedTabs, $isAdmin)
    {
        ...
        $activeForums = array_map(function ($a) {
            return $a['id'];
        }, $activeForums);
        $Grouprights = array_map(function($a) {
            return $a['rights'][0];
        }, $this->container->get('contact')->get('clubRoleRightsRoleWise'));
 ...
 /**
  * To check the contact count
  * @param integer $addCount
  * @return int
  */
    public function checkContactCount($addCount=1)
    {
        $adminEntityManager = $this->container->get("fg.admin.connection")->getAdminEntityManager();
        $activeContactcount = $adminEntityManager->getRepository('AdminUtilityBundle:FgClub')->getActiveContactCount($this->clubId);
        
        $status = 1;
        $addCountValue = $activeContactcount[0]['activeContactCount']+$addCount;
        if ($addCountValue > $this->container->getParameter('clubcontactCount')) {
            $status = 0;
        }
